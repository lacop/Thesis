...
Súčasťou práce je priložené CD obsahujúce zdrojový kód \citep{attsource} tohto programu a tiež skompilovanú verziu \citep{attbin}. Aplikácia na spustenie požaduje \emph{JVM}\footnote{\emph{Java Virtual Machine}, dostupné na \url{https://www.java.com/en/download/}}. Spustenie je možné vykonať spustením súborov \inlcode{start.sh} prípadne \inlcode{start.bat}, podla operačného systému. Dostupné sú tiež individuálne vizualizácie vo forme \emph{Java appletov}, ktoré je možné spustiť otvorením súboru \inlcode{index.html} v preferovanom internetovom prehliadači.
