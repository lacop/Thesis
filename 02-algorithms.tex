...
Majme pole $A$ veľkosti $|A|=N$ a označme jeho prvky $A = \{a_1,\dotsc,a_N\} \in X^N$. Chceme vypočítať hodnotu $f_g(A)$, kde $g: X \times Y \rightarrow Y$ je agregačná funkcia, $g_0 \in Y$ je počiatočná hodnota a $f_g: 2^X \rightarrow Y$ je\footnote{$2^X$ označuje potenčnú množinu množiny $X$} rozšírenie agregačnej funkcie definované následovne:
\[
\begin{aligned}
...
\begin{figure}
    ...
    \caption{Schematické znázornenie rekurzívneho delenia pri \vEB usporiadaní. Podstromy $\tau_0,\dotsc,\tau_k$ sa uložia do súvislého úseku pamäte.}
...
\begin{figure}
    ...
    \caption{Usporiadané pole veľkosti $N$. Strom nad blokmi je len imaginárny a nezostrojuje sa v pamäti.}
...
Ďalej \emph{hustotou}, $0 \le d(v) \le 1$, označíme $d(v) = \frac{o(v)}{c(v)}$. Zvoľme ľubovolné konštanty $\rho_d, \tau_d, \rho_0, \tau_0$ spĺňajúce
\[
...
\begin{figure}
    ...
    \caption{Dynamický strom, ktorý vznikne spojením statického stromu vo \vEB usporiadaní a usporiadaného pola. Šipky znázorňujú spárovanie listov a položiek pola.}
...
\subsubsection{Stredná časž - najbližší spoločný predok}
...
\[
\bigO{\frac{\log_B{N} + \frac{\log^2{N}}{B}}{\log{N}}} = \bigO{\log{N}{B}}
\]
